        ...
        <tbody>
          <tr v-for="(item,index) in arr" :key="index">
            ...
            <td v-if="index + 1 === arr.length">
              <b-button variant="primary" @click="addLine(item)">+</b-button>
            </td>
            <td v-else>
              <b-button variant="danger" @click="removeData(index)">-</b-button>
            </td>
          </tr>
        </tbody>
      </table>
      <div>
        <b-button style="float:centre" variant="success" @click="postData(form)">Submit</b-button>
      </div>
    </b-container>
  </div>
</template>
